...
class LoanViewSet(viewsets.ModelViewSet):
    ...
    def extend_due_date(self, request, pk=None):
        serializer = ExtendDateSerializer(data=request.data)
        # days = request.data.get("additional_days")
        serializer.is_valid(raise_exception=True)

        loan = self.get_object()
        if loan.due_date < timezone.now().date():
            raise ValidationError("Loan is overdue already")

        loan.due_date = F("due_date") + serializer.validated_data.get("additional_days")
        loan.save()
